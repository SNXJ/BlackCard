...
public class MyOrderCenterOtherHotelAdapter extends BaseMultiItemQuickAdapter<OrderCenterHotelRecyclerModel.PdBean, BaseViewHolder> {
     ...
     */
    public MyOrderCenterOtherHotelAdapter(List<OrderCenterHotelRecyclerModel.PdBean> data) {
        super(data);
    ...
    @Override
    protected void convert(BaseViewHolder helper, OrderCenterHotelRecyclerModel.PdBean item) {
        Glide.with(mContext).load(item.getSHOWIMAGES()).into((ImageView) helper.getView(R.id.my_order_img));
        Glide.with(mContext).load(item.getSHOWIMAGES()).into((ImageView) helper.getView(R.id.my_order_img));
        helper.setText(R.id.my_order_title_tv, item.getHOTELDETNAME());
    }
}
