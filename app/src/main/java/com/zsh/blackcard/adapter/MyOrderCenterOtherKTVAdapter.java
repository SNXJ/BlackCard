...
public class MyOrderCenterOtherKTVAdapter extends BaseMultiItemQuickAdapter<OrderCenterKTVRecyclerModel.PdBean, BaseViewHolder> {
     ...
     */
    public MyOrderCenterOtherKTVAdapter(List<OrderCenterKTVRecyclerModel.PdBean> data) {
        super(data);
    ...
    @Override
    protected void convert(BaseViewHolder helper, OrderCenterKTVRecyclerModel.PdBean item) {
        Glide.with(mContext).load(item.getSHOWIMAGES()).into((ImageView) helper.getView(R.id.my_order_img));
        Glide.with(mContext).load(item.getSHOWIMAGES()).into((ImageView) helper.getView(R.id.my_order_img));
        helper.setText(R.id.my_order_title_tv, item.getKTVDETTITLE());
    }
}
