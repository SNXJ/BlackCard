...
import com.zsh.blackcard.custom.LiveMusicDialog;

import java.util.ArrayList;

public class LiveMusicAdapter extends RecyclerView.Adapter<LiveMusicAdapter.MusicViewHolder> {

    private Context mContext;

    private LiveMusicDialog.OnItemClick mOnItemClick = null;
    ...
    public MusicViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.live_music_item_info, parent, false);
    ...
    public void onBindViewHolder(final MusicViewHolder holder, final int position) {
        ...
        if(mPosition == position) {
            ...
            holder.tvMusicName.setTextColor(mContext.getResources().getColor(R.color.bg_org));
            if(position != 0) {
                holder.tvLoop.setVisibility(mIsLoop ? View.VISIBLE : View.GONE);
            }
        } else {
            holder.tvMusicCheck.setVisibility(View.INVISIBLE);
            holder.tvMusicName.setTextColor(mContext.getResources().getColor(R.color.ff92_text));
    ...
    public void setOnItemClick(LiveMusicDialog.OnItemClick onItemClick) {
