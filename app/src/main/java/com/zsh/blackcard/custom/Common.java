...
//    }
    ...
    public static ArrayList<LiveMusicAdapter.MusicInfo> getResource() {
        ArrayList<LiveMusicAdapter.MusicInfo> list = new ArrayList<>();
        if (new File(SD_DIR + RESOURCE_DIR).exists()) {
            ...
            if (null != files && files.length > 0) {
                for (int i = 0; i < files.length; i++) {
                    LiveMusicAdapter.MusicInfo musicInfo = new LiveMusicAdapter.MusicInfo();
