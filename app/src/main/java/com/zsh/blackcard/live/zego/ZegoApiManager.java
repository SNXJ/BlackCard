...
//        }
    ...
    private void init(long appID, byte[] signKey){
        ...
        // 初始化sdk
        boolean ret = mZegoLiveRoom.initSDK(appID, signKey, BaseApplication.getApplication());
        if(!ret){
            // sdk初始化失败
            Toast.makeText(BaseApplication.getApplication(), "Zego SDK初始化失败!", Toast.LENGTH_LONG).show();
        } else {
            // 初始化设置级别为"High"
