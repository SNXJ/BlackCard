...
import com.zsh.blackcard.live.zego.BasePlayActivity;
...
public class SingleAnchorPlayActivity extends BasePlayActivity {
     ...
     * @param roomInfo 房间信息
     */
    public static void actionStart(Activity activity, LiveInfoListModel.DataBean.RoomListBean roomInfo) {
        Intent intent = new Intent(activity, SingleAnchorPlayActivity.class);
        intent.putExtra(Constants.ROOM_ID, roomInfo.room_id);

        ArrayList<String> streamList = getStremListFromRoomInfo(roomInfo);
        intent.putStringArrayListExtra(Constants.LIST_STREAM, streamList);
